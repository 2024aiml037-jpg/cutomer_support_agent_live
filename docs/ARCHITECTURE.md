...
4. Runs all of the above through a LangChain `create_agent` agent (LangGraph runtime) backed by a Groq LLM to produce a personalised reply draft.
...
| Manual draft generation | `POST /api/tickets/{ticket_id}/generate-draft` |
...
| Memory probe | `GET /api/customers/{customer_id}/memory-search` |
...
routers/
  ...
  memory.py          GET /api/customers/{customer_id}/memories
...
| Service | Role |
|---|---|
| `SupportCopilot` | Orchestrates the LangChain agent; owns `KnowledgeBaseService` and `CustomerMemoryStore` |
| `DraftService` | Serialises tickets/drafts; runs background and manual draft generation |
...
SupportCopilot.generate_draft(ticket, customer)
       ...
       │           scopes: [customer_email, company::<normalized company name>]
       │
       ...
       │
       ├─4─► agent.invoke() (LangChain create_agent)
...
### 4.2 Agent Runtime (LangChain `create_agent`)

The agent is built with `langchain.agents.create_agent`, which runs on the LangGraph runtime.

- **Model**: `ChatGroq` (default `llama-3.1-8b-instant`, configurable)
- **Checkpointer**: `MemorySaver` (in-process; `thread_id` = `ticket::{ticket_id}`, falling back to `ticket::{customer_email}`)
...
2. A `BackgroundTask` calls `SupportCopilot.save_accepted_resolution()`, swallowing any memory-save error so acceptance always succeeds.
3. The resolution (subject, description, draft, extracted entity links such as endpoints, HTTP status codes, regions, integrations, and plan tiers) is saved to Mem0 under both the customer's email and the `company::<normalized company name>` user IDs.
...
| Scope | user_id |
|---|---|
| Individual | `customer_email` (lower-cased) |
| Org-level | `company::<normalized company name>` derived from the customer's `company` field |
...
| `POST` | `/api/tickets` | Create ticket; optionally enqueues background draft (`auto_generate`) |
| `POST` | `/api/tickets/{id}/generate-draft` | Synchronously (re)generate a draft for a ticket |
...
| `POST` | `/api/knowledge/ingest` | Re-ingest `knowledge_base/` into ChromaDB (optional `clear_existing`) |
| `GET` | `/api/customers/{customer_id}/memories` | List Mem0 memories for a customer (both scopes) |
| `GET` | `/api/customers/{customer_id}/memory-search` | Semantic search over a customer's memories (`query`, `limit`) |
...
| LangChain `create_agent` (LangGraph runtime) over a raw ReAct loop | Built-in checkpointing, structured message history, deterministic tool invocation |
...
| Dual memory scope (email + company name) | Lets the agent benefit from org-level patterns (e.g. all tickets from the same company) without coupling individual user data |
...
└── e2e/                    Playwright dashboard golden-path UI test
```
